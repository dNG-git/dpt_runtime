...
Exception traceback list
        ...
        if (traceback is not None):
            self._trace_list = (traceback.format_stack()[:-1]
                                   if (exc_traceback is None) else
                                   traceback.format_tb(exc_traceback)
                                  )
        elif (exc_traceback is not None): self._trace_list = [ repr(exc_traceback) ]
    #
...
:since:  v1.0.0
        ...
        if (hasattr(_super, "with_traceback")): _super.with_traceback(tb)
